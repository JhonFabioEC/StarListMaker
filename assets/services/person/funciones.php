...
function subir_imagen()
{
    if (isset($_FILES['image'])) {
        ...
        $nombre = uniqid();
        $ubicacion = "$root/StarListMaker/assets/img/user/person/";
...
function obtener_nombre_imagen($id)
{
    ...
    $stmt = $connection->prepare('SELECT image FROM view_person WHERE user_id=' . $id . ';');
    $stmt->execute();
...
{
    $connection = new Connection();
    $stmt = $connection->prepare("SELECT * FROM public.view_person WHERE person=" . $_SESSION['id'] . ";");
    $stmt->execute();
    $resultado = $stmt->fetchAll();
